...
sudo apt-get install -qq -o=Dpkg::Use-Pty=0 -y keepassxc ca-certificates unrar-free transmission aisleriot kdiff3 krename zip p7zip-full breeze-icon-theme gnome-keyring krusader code
(($?)) && read -n1 -r -p "[sudo apt-get install keepassxc ca-certificates unrar-free transmission aisleriot kdiff3 krename zip p7zip-full breeze-icon-theme gnome-keyring krusader code] FAILED!" key
...
(($?)) && read -t 10 -n1 -r -p "[./veracrypt/setup_veracrypt.sh] FAILED!" key

# install RClone
chmod +x ./rclone/setup_rclone.sh
(($?)) && read -n1 -r -p "[chmod +x ./rclone/setup_rclone.sh] FAILED!" key
./rclone/setup_rclone.sh
(($?)) && read -t 10 -n1 -r -p "[./rclone/setup_rclone.sh] FAILED!" key
...
(($?)) && read -t 10 -n1 -r -p "[./cryptomator/cryptomator.sh] FAILED!" key

# Setup nord VPN
chmod +x ./vpn/setup_nordvpn.sh
(($?)) && read -n1 -r -p "[chmod +x ./vpn/setup_nordvpn.sh] FAILED!" key
./vpn/setup_nordvpn.sh
(($?)) && read -t 10 -n1 -r -p "[./vpn/setup_nordvpn.sh] FAILED!" key
...
(($?)) && read -t 10 -n1 -r -p "[./cryptomator/hardening.sh] FAILED!" key

# Setup screen saver and blanking (fix alt-tab)
chmod +x ./tweaks/tweaks.sh
(($?)) && read -n1 -r -p "[chmod +x ./tweaks/tweaks.sh] FAILED!" key
./tweaks/tweaks.sh
(($?)) && read -t 10 -n1 -r -p "[./tweaks/tweaks.sh] FAILED!" key

# update
sudo apt-get update -qq -o=Dpkg::Use-Pty=0 
(($?)) && read -t 10 -n1 -r -p "[sudo apt-get update] FAILED!" key
sudo apt-get autoremove -qq -o=Dpkg::Use-Pty=0 -y
(($?)) && read -t 10 -n1 -r -p "[sudo apt-get autoremove -qq -o=Dpkg::Use-Pty=0 -y] FAILED!" key
